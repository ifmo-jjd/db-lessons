package ru.ifmo.db.additional;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.type.CollectionType;
import ru.ifmo.db.additional.jackson.Cat;
import ru.ifmo.db.additional.jackson.Category;
import ru.ifmo.db.additional.jackson.Color;
import ru.ifmo.db.additional.validation.Task;
...
public class App {
    ...
    private static void runValidation(){
        Task task = new Task();
        task.setTitle("Важная задача");
        task.addMember("Глеб");
        task.setDifficultyLevel(10);

        validate(task);
    }

    public static void validate(Task car) {
        ValidatorFactory factory = Validation.buildDefaultValidatorFactory();
        Validator validator = factory.getValidator();

        Set<ConstraintViolation<Task>> cvs = validator.validate(car);
